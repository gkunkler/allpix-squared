...
filePaths = ['data_cr1_d1_e_100.root','data_cr1_d1_eh_100.root','data_cr1_d1_ehm_100.root']
names = ['Electrons only','Electrons & Holes','Electrons, Holes, Mirror Charges']

hist_fig, hist_axs = plt.subplots(3,1,constrained_layout=True, figsize=(8,6), sharex=True)
...
for fileIndex in range(len(filePaths)):
    ...
    if (containsDC or containsPC) and num_events <= 5:
        ...
        if containsDC and containsPC: 
            hist_fig.suptitle("Z Positions of Deposited and Propagated Charges")
        elif containsDC: 
            hist_fig.suptitle("Z Positions of Deposited Charges")
        else: 
            hist_fig.suptitle("Z Positions of Propagated Charges")
