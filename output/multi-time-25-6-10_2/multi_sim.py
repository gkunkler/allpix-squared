...
num_charge_list = [50,150,250,350] # [50,150,250,450,550,650,750]
coulomb_distance_limits = [1e-5,5e-5,5e-4]# [1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3, 1e-1] #cm
repeats = 2 # 3
time_steps = [0.1,0.5]# [0.01, 0.05, 0.1, 0.5] #ns
...
for i in range(repeats):
    ...
    for coulomb_distance_limit in coulomb_distance_limits:
        ...
        for num_charges in num_charge_list:
            ...
            for time_step in time_steps:
                ...
                for propagation_config in [[1,1,1,0,0], [1,1,1,0,1]]: #[d,cr,e,h,m]
                    ...
                    allpix.CONFIGURATION_DESCRIPTION = f"{"e" if propagate_electrons == 1 else ""}{"h" if propagate_holes == 1 else ""}{"m" if include_mirror == 1 else ""}_{bias_voltage}V_{integration_time}x{time_step}ns_{num_charges}-{spot_size}um_{coulomb_distance_limit}cm_{coulomb_field_limit}Vcm".replace(".","p") #replace if there are "." in the path
