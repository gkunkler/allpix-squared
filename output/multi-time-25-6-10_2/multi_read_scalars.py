...
for ax, i, j, filter_1_value, filter_2_value in zip(axes.flatten(), both_indices[0], both_indices[1], both_filters[0], both_filters[1]):
    ...
    filter_1_title = f'{filter_1_name}: {filter_1_value}{f' [{filter_1_units}]' if not filter_1_units == '' else ''}' 
    filter_2_title = f'{filter_2_name}: {filter_2_value}{f' [{filter_2_units}]' if not filter_2_units == '' else ''}'
